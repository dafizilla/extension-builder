...
{
    diff --old-group-format='' --new-group-format='' $OUT_DIR/${LANG_REF}_sorted.txt $OUT_DIR/${1}_sorted.txt >$OUT_DIR/diff_result.txt
    if [ "$?" -eq "1" ];
    then
        echo $1 differs
...
function make_result_for_lang()
{
    ...
    do
        awk -f $BASEDIR/find_untraslated_strings.awk $i >>$OUT_DIR/curr_tmp.txt
    done
...
if [ -d $LOCALE_DIR ];
then
	EXT_NAME=`ls $LOCALE_DIR/en-US/`
	show_diff_per_file $*
else
	echo Unable to find directory $LOCALE_DIR
fi
